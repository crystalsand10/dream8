...
% sfam is the sorted list of [pa ni], where "pa" is the current set of
% parents being considered for node "i"

% ordAll is the sorted list based on the indeces of the low to high values

% ord is the index of the sfam list containing the node "i"
ord = find(sfam==ni); % the dimension we'll sum over later

% sfamsz is the arity of the parent nodes being considered
...
if ~isempty(priorCache)  % if the priorCache is NOT empty
	prior = priorCache{length(sfam)};
else  % if the priorCache IS empty
	psz = prod(nodeArity(pa));  % the product of the arities of the parent nodes being considered
	% prior = (alpha/psz) * mk_stochastic(myones(famsz)); too slow (slower
	% by a factor of 2 than what follows)
	
	if length(sfamsz)==1  % if only one parent being considered
        
        % psz*sfamsz(ord) represents the product of the arities of the
        % parent nodes and child nodes (e.g.,
        % arity(pa1)*arity(pa2)*arity(pa3)*arity(child node))
