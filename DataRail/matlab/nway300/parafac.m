function [Factors,it,err,corcondia,PercentExpl]=parafac(X,Fac,varargin);
...
% $ Version 2.14 $ Feb 2006 $ Fixed bug in sign-swicth when loadings are
% fixed $ RB $ Not compiled $
...
nanTest = isnan(X);
if sum(nanTest(:)) == 0
    [Factors,it,err,corcondia,PercentExpl]=parafacMain(X,Fac,varargin{:});
    return
end
% Check for "slabs" of NaNs and temporarily remove them from X
shape = size(X);
n = ndims(X);
idx0 = repmat({':'}, 1, n);
emptyTest = cell(1, n);
...
for i=1:n
    emptyTest{i} = false(1, shape(i));
    idx = idx0;
    % Check each index
    for j=shape(i):-1:1
        idx{i} = j;
        % Check entire "slab"
        if all(reshape(nanTest(idx{:}), 1, []))
            % Store index of empty "slabs"
            emptyTest{i}(j) = true;
        end
    end
end
% Run like normal, if no empty slabs
if ~any([emptyTest{:}])
    [Factors,it,err,corcondia,PercentExpl]=parafacMain(X,Fac,varargin{:});
    return
end
...
[Factors0,it,err,corcondia,PercentExpl]=parafacMain(X,Fac,varargin{:});
Factors = cell(1,n);
for i=1:n
    Factors{i} = zeros(shape(i),Fac);
    Factors{i}(~emptyTest{i},:) = Factors0{i};
    Factors{i}(emptyTest{i}) = nan;
end
% Reinsert NaNs in Factors

function [Factors,it,err,corcondia,PercentExpl]=parafacMain(X,Fac,Options,const,OldLoad,FixMode,Weights);
NumbIteraInitia=20;
DimX = size(X);
...
persistent OptionsDefault
if isempty(OptionsDefault)
    load noptiopf OptionsDefault;
end
...
while (((f>crit) || (norm(connew-conold)/norm(conold)>MissConvCrit) || ...
        ConstraintsNotRight) && it<maxit) || ~nonneg_obeyed
  ...
  if it/showfit-round(it/showfit)==0
    if showfit~=-1,
      ...
      if DoWeight==0
        ...
        PercentExpl=100*(1 - nansum( X(:)-model(:) ).^2/SSX );
      end
