...
num_lv = o.num_lv;
   ...
   while (norm(t-tgl)/norm(t))>crit&it<maxit
      ...
      % w=X'u
      [wloads,wkron] = Xtu(o.X,u,o.MissingX,o.missX,o.Jx,o.DimX,o.ordX);
      
      % t=Xw
      if o.MissingX
         for i=1:o.I,
            m = find(o.missX(i,:));
            t(i)=o.X(i,m)*wkron(m)/(wkron(m)'*wkron(m));
         end
      else
         t=o.X*wkron;
      end
      ...
      % w=X'u
      [qloads,qkron] = Xtu(o.Yres,t,o.MissingY,o.missY,o.Jy,o.DimY,o.ordY);
      % u=yq
      if o.MissingY
         for i=1:o.I
            m = find(o.missY(i,:));
            u(i)=o.Yres(i,m)*qkron(m)/(qkron(m)'*qkron(m));
         end
      else
         u=o.Yres*qkron;
      end
...
%    qloads = Factors(2:end);
   ...
   for i = 1:o.ordX-1
      if num_lv == 1
         ...
         o.W{i} = [o.W{i} wloads{i}];
      end
   end
   o.U=[o.U u];
   for i = 1:max(o.ordY-1,1)
      if num_lv == 1
         o.Q{i} = qloads{i};
      else
         o.Q{i} = [o.Q{i} qloads{i}];
      end
   end
   o.Qkron = [o.Qkron qkron];
  
   % Make core arrays
   if o.ordX>1
      o.Xfac{1}=o.T;o.Xfac(2:o.ordX)=o.W;
      o.Core{num_lv} = calcore(reshape(o.X,o.DimX),o.Xfac,[],0,1);
   else
      o.Core{num_lv} = 1;
   end
%   if ordY>1
...
%   end
   ...
   o.B(1:num_lv,num_lv)=inv(o.T'*o.T)*o.T'*o.U(:,num_lv);
   ...
   % Make X model
   if o.ordX>2
      o.Wkron = kron(o.W{end},o.W{end-1});
   else
      o.Wkron = o.W{end};
   end
   for i = o.ordX-3:-1:1
      o.Wkron = kron(o.Wkron,o.W{i});
   end
   if num_lv>1
      xmodel=o.T*reshape(o.Core{num_lv},num_lv,num_lv^(o.ordX-1))*o.Wkron';
   else
      xmodel = o.T*o.Core{num_lv}*o.Wkron';
   end
 ...
 %  end
 o.ypred=o.T*o.B(1:num_lv,1:num_lv)*o.Qkron';
 o.Ypred(:,num_lv) = o.ypred(:); % Vectorize to avoid problems with different orders and the de-vectorize later on
   
   o.Xres=o.X-xmodel; 
   o.Yres=o.Y-o.ypred;
   if o.MissingX
      o.ssx=[o.ssx;sum(sum(o.Xres(find(o.missX)).^2))];
   else
      o.ssx=[o.ssx;sum(sum(o.Xres.^2))];
   end
   if o.MissingY
      o.ssy=[o.ssy;sum(sum((o.Y(find(o.missY))-o.ypred(find(o.missY))).^2))];
   else
      o.ssy=[o.ssy;sum(sum((o.Y-o.ypred).^2))];
   end
% end
% ypred = reshape(o.Ypred',[size(o.Ypred,2) o.DimY]);
% ypred = permute(ypred,[2:o.ordY+1 1]);
% 
% o.ssx= [ [o.SSX(1);o.ssx] [0;100*(1-o.ssx/o.SSX(1))]];
% o.ssy= [ [o.SSy(1);o.ssy] [0;100*(1-o.ssy/o.SSy(1))]];
% 
...
%    disp('   ----    -------    -------')
%    ssq = [(1:o.Fac)' ssx(2:o.Fac+1,2) ssy(2:o.Fac+1,2)];
...
%       end
%       for i=1:Fac
%         sR = R(:,1:i)*o.B(1:i,1:i)*diag(o.Q{1}(iy,1:i));
%         ssR = sum( sR',1)';
%         o.reg{iy,i} = reshape( ssR ,dd);
%       end       
%     end
%     
% %   end
