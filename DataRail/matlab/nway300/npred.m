function [ypred,T,ssX,Xres,ypred0]=npred(X0,Fac,Xfactors,Yfactors,Core,B,show,processing)
...
if ~exist('processing','var') || isempty(processing)
    ...
    for i=1:numel(processing)
        processing(i) = setParameters(defaultProcessing, processing(i));
    end
end
...
% Make sure we post-process Y data
if processing(2).iter > 0
    processing(2).iter = -processing(2).iter;
end

% Convert to old notation
X = nprocess(X0,processing(1));
Dimx = size(X);
...
ypred0=T*B(1:Fac,1:Fac)*Q(:,1:Fac)';
Dimy = DimY;
Dimy(1) = Dimx(1);
ypred0 = reshape(ypred0, Dimy);
ypred = nprocess(ypred0, processing(2));
