function data = createDataCube(varargin)
% createDataCube('param1',value1,...) creates a dataCube structure
%  
%  data = createDataCube(varargin)
%
%--------------------------------------------------------------------------
% INPUTS: 
%
...
%    Data.data(v.TimeC) = createDataCube(...
...
%
%   Data.data(v.Bool) = createDataCube(...
%       'Name', 'Boolean', ...
...
%
%    Data.data(v.Relat) = createDataCube(...
...
data = struct('Name','', 'Info', '', 'Labels', [], 'Value', [], ...
    'SourceData', '', 'Code', [], 'CodeHashArray', [], 'Parameters', struct);
%% Parse arguments
% Default to print warnings
PrintWarnings = true;
% Check number of arguments
if ~strcmp(varargin{6},'CustomMath')
    if mod(nargin,2) ~= 0
...
if ~ischar(data.SourceData) && ~iscellstr(data.SourceData)
    ...
    if isempty(data.Labels)
        ...
        elseif PrintWarnings
            warning('Source data is an old-style data cube without a Labels structure');
        end
    end
end
...
%        try
            ...
            elseif isempty(data.Value)
                if size(varargin,2)>10    % for function customMath
                    data = callFun(fhandle, sourceData, data, varargin{11});
                else
                    data = callFun(fhandle, sourceData, data);
                end
            elseif PrintWarnings
                warning('createDataCube:ValueCodeConflict',...
...
%Check that the cube is valid - for most cases
if PrintWarnings&&exist('code','var')&&~(ischar(code)&&strcmp(code,'CreateCNOData'))
    checkDataCube(data);
end


function data = callFun(fhandle, sourceData, data, Compendium)
...
try
    ...
    if strcmp(func2str(fhandle),'Normalize')
        data.Parameters = p;
    end
    ...
    if strcmp(func2str(fhandle),'CustomMath')
        data.Value = fhandle(sourceData.Value, Parameters, Compendium);
    else
